...
angular.module('copayApp.controllers')
	.controller('microblogController', function ($scope, $rootScope, $timeout, $interpolate, $filter, $modal, $log, $state, $stateParams, $http, $location, notification, isCordova, profileService, lodash, configService, storageService, gettext, gettextCatalog, nodeWebkit, addressService, confirmDialog, animationService, addressbookService, correspondentListService, newVersion, autoUpdatingWitnessesList, go, aliasValidationService, localStorageService, mylocalStorageService) {
		...
		this.protocol = conf.program.replace(/LCP Mobile/i, 'LCP Mobile');
		...
		$scope.pagetype = $stateParams.flagId;
		...
		// this.getlcpvalue=function(){
		var _that = this;
		indexScope.arrBalances.filter(function (balance) {
		...
		function getCost() {
			var strCost = '';
			http.get('http://weibo.lcp.talkingcake.cn/api/other/getCosts', function (res) {
				res.on('data', function (chunk) {
					strCost += chunk;
				});
				res.on("end", function () {
					// alert(strCost);
					var costdata = JSON.parse(strCost);
				
					//  $scope._amount=costdata.data.weibo;
					if($scope.pagetype == 'comment' || $scope.pagetype == 'reply'){
                         $scope._amount=costdata.data.comment;
					}else if($scope.pagetype == 'weibo'){
						 $scope._amount=costdata.data.weibo;
					}
				});
		...
		// 获取销毁地址
		function getRuinAddress() {
			var strAddress = '';
			http.get('http://weibo.lcp.talkingcake.cn/api/other/getRuinAddress', function (res) {
				res.on('data', function (chunk) {
					strAddress += chunk;
				}); 
				res.on("end", function () {
					// alert(strAddress);
					var adressdata = JSON.parse(strAddress);
					 if($scope.pagetype=='weibo'|| $scope.pagetype=='dislike'){
						$scope._address = adressdata.data.ruinAddress;
					 }else if($scope.pagetype=='like'|| $scope.pagetype=='comment' || $scope.pagetype=='reply'){
						$scope._address = adressdata.data.platform;
					 }
					// $scope._address = adressdata.data.ruinAddress;
				});
			}).on("error", function (err) {
		...
		$scope.changecomment = function (_mycomment) {
			$timeout(function () {
				$scope._mycomment = _mycomment;
			}, 100)
		}

		$scope.changeweibo = function (_weibocont) {
			$timeout(function () {
				$scope._weibocont = _weibocont;
			}, 100)
		}
	    ...
	    $scope.checkvalue=function(x){
			...
			// $timeout(function () {
			// 	if(x===parseInt(x)){
			// 		$scope._amount=$scope._amountalias=parseInt(x);
			// 	}else if(!x){
		...
		$scope.changeselectbar=function(num){
				$scope.selectbar=num;
			}
		...
		function sendlike(type) {
			...
			var obj = {
				"fromUserAddress": localAddress,
				"likeWeiboId": weiboId,
				"tractionId": $scope.transitionid,
				"cost": $scope._amount,
				"type":type
			}
			...
			$http({
				method: 'post',
				url: 'http://weibo.lcp.talkingcake.cn/api/weiboLike/addWeiboLike',
				data: obj,
			}).then(function success(res) {
				//  alert(res.data.msg);
				window.plugins.toast.showShortCenter(gettextCatalog.getString(res.data.msg));
				// go.walletHome();
		...
		function sendcomment() {
			var obj = {
			    "nickName":$scope._nickName,
				"weiboId": weiboId,
				"commentsContent": $scope._mycomment,
				"fromUserAddress": localAddress,
				"cost": $scope._amount,
				'tractionId': $scope.transitionid
			}
			...
			$http({
				method: 'post',
				url: 'http://weibo.lcp.talkingcake.cn/api/weiboComments/addComment',
				data: obj,
			}).then(function success(res) {
				//  alert(res.data.msg);
				window.plugins.toast.showShortCenter(gettextCatalog.getString(res.data.msg));
		...
		function sendweibo() {
			...
			$http({
				method: 'post',
				url: 'http://weibo.lcp.talkingcake.cn/api/weiboContent/sendWeiBo',
				data: obj,
			}).then(function success(res) {
				//  alert(res.data.msg);
				window.plugins.toast.showShortCenter(gettextCatalog.getString(res.data.msg));
		...
		function sendreply() {
			...
			$http({
				method: 'post',
				url: 'http://weibo.lcp.talkingcake.cn/api/replyComment/addOtherComment',
				data: obj,
			}).then(function success(res) {
				//  alert(res.data.msg);
				window.plugins.toast.showShortCenter(gettextCatalog.getString(res.data.msg));
		...
		this.shareAddress = function (addr) {
			if (isCordova) {
				...
				window.plugins.socialsharing.shareWithOptions({ message: "My LCP address " + self.protocol + ':' + addr, subject: "My LCP address"/*, url: self.protocol +  ':' + addr*/ }, function () { }, function () { });
			}
		};
		...
		this.openCustomizedAmountModal = function (addr) {
			...
			var ModalInstanceCtrl = function ($scope, $modalInstance) {
				...
				$scope.protocol = conf.program.replace(/LCP Mobile/i, 'LCP Mobile');
		...
		this.submitPayment = function () {
			// alert($scope._amount);
			// return false;
			if ($scope.pagetype == 'comment') {
				if (!$scope._mycomment) {
					window.plugins.toast.showShortCenter(gettextCatalog.getString('请输入评论内容'));
					return false;
				}
			}else if($scope.pagetype == 'reply'){
				if (!$scope._mycomment) {
					window.plugins.toast.showShortCenter(gettextCatalog.getString('请输入评论内容'));
					return false;
				}
				...
				if (!$scope._weibocont) {
					window.plugins.toast.showShortCenter(gettextCatalog.getString('请输入微博内容'));
					return false;
				}
			}else if($scope.pagetype == 'like' || $scope.pagetype=='dislike'){
						...
						fc.sendMultiPayment(opts, function (err, unit, mnemonics) {
							...
							if (recipient_device_address) { // show payment in chat window
								...
								if (binding && binding.reverseAmount) { // create a request for reverse payment
									...
									var paymentRequestCode = 'LCP:' + my_address + '?amount=' + binding.reverseAmount + '&asset=' + encodeURIComponent(binding.reverseAsset);
							...
							$rootScope.$emit('Local/SetTab', 'history');
							$scope.transitionid = unit;
							if ($scope.pagetype == 'like') {
								sendlike(1);
								go.walletHome();
								
				            
							}else if($scope.pagetype == 'dislike'){
								sendlike(0);
								go.walletHome();
								

							} else if ($scope.pagetype == 'comment') {
								sendcomment();
								go.walletHome();
							} else if ($scope.pagetype == 'weibo') {
								sendweibo();
		...
		this.submitData = function () {
			...
			var errored = false;
			$scope.myhome.feedvaluespairs.forEach(function (pair) {
				if (value[pair.name]) {
			...
			profileService.requestTouchid(function (err) {
				...
				if (app == "attestation") {
					value = {
						address: $scope.myhome.attested_address,
						profile: value
					};
				}
				if (app == "poll") {
					value = {
						question: $scope.myhome.poll_question,
			...
			$timeout((function () {
				switch (app) {
					...
					case 'attestation':
						...
						$scope.myhome.attested_address = dataPrompt.address;
					...
					case 'poll':
						...
						$scope.myhome.poll_question = dataPrompt.question;
				...
				for (var key in dataPrompt) {
					...
					$scope.myhome.feedvaluespairs.push(app === 'poll' ? { name: value, value: 'anything' } : { name: key, value: value });
				}
				// this.switchForms();
				//	$timeout(function () {
				//		$rootScope.$digest();
		...
		this.resetForm = function () {
			...
			// $rootScope.$emit('Local/ShowAlert', '999999', );
		...
		this.openInExplorer = function (unit) {
			...
			var testnet = myhome.isTestnet ? 'testnet' : '';
			var url = 'http://' + testnet + 'explorer.lovecoinplus.com/#' + unit;
