...
if [ ! -d $PROJECT ]; then
	...
	cordova create ../../obytebuilds/project-$1 org.lovechainplus.wallet lovechainplus
	checkOK
...
#  checkOK
	...
	cordova plugin add cordova-plugin-inappbrowser@2.0.1
	checkOK

	cordova plugin add cordova-plugin-x-toast && cordova prepare
	checkOK

	cordova plugin add https://github.com/tgensol/CordovaClipboard
	checkOK
...
#	checkOK
	...
	if [ $CURRENT_OS == "IOS" ]; then
		...
		#cordova plugin add android-referrer-plugin
		checkOK
	fi
